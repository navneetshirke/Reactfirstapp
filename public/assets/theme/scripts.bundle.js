...
var KTApp = function() {
    ...
    var initTooltip = function(el) {
        ...
        $(el).tooltip({
            trigger: trigger,
            template: '<div className="tooltip ' + theme + ' ' + width + '" role="tooltip">\
    ...
    return {
        ...
        block: function(target, options) {
            ...
            var spinner = '<span className="spinner ' + version + ' ' + state + ' ' + size + '"></span';

            if (options.message && options.message.length > 0) {
                var classes = 'blockui ' + (options.shadow === false ? 'blockui' : '');

                html = '<div className="' + classes + '"><span>' + options.message + '</span>' + spinner + '</div>';
    ...
    var Plugin = {
        ...
        */
        ...
        createSubmenuDropdownClickDropoff: function(el) {
            ...
            var dropoff = document.createElement('<div className="menu-dropoff" style="background: transparent; position: fixed; top: 0; bottom: 0; left: 0; right: 0; z-index: ' + zIndex + '"></div>');
...
var KTWizard = function(elementId, options) {
    ...
    var defaultOptions = {
        startStep: 1,
        clickableSteps: false, // to make steps clickable this set value true and add data-wizard-clickable="true" in HTML for className="wizard" element
        navigation: true
    };
					...
					if (!$(detailRow).hasClass(pfx + 'datatable-row-detail')) {
						...
						$(clonedCells).each(function() {
							...
							if (typeof column === 'undefined' || column.visible !== false) {
								$(detailSubTable).
										append($('<tr className="' + pfx + 'datatable-row"></tr>').
												append($('<td className="' + pfx + 'datatable-cell"></td>').append($('<span/>').append(column.title))).
												append(this));
							}
						});
						...
						$(datatable.tableBody).find('.' + pfx + 'datatable-row').each(function() {
							...
							if ($(datatable.tableHead).find('.' + pfx + 'datatable-toggle-detail').length === 0) {
								...
								$(datatable.tableHead).
									...
									first().
									prepend('<th className="' + pfx + 'datatable-cell ' + pfx + 'datatable-toggle-detail"><span></span></th>');
								$(datatable.tableFoot).
									find('.' + pfx + 'datatable-row').
									first().
									prepend('<th className="' + pfx + 'datatable-cell ' + pfx + 'datatable-toggle-detail"><span></span></th>');
...
var KTLayoutChat = function () {
	...
	var _handeMessaging = function(element) {
		...
		html += '	<div>';
		html += '		<span className="text-muted font-size-sm">2 Hours</span>';
		html += '		<a href="#" className="text-dark-75 text-hover-primary font-weight-bold font-size-h6">You</a>';
		html += '	</div>';
		html += '	<div className="symbol symbol-circle symbol-40 ml-3">';
		html += '		<img alt="Pic" src="assets/media/users/300_12.jpg"/>';
		html += '	</div>';
		html += '</div>';
		html += '<div className="mt-2 rounded p-5 bg-light-primary text-dark-50 font-weight-bold font-size-lg text-right max-w-400px">' + textarea.value + '</div>';
		...
		setTimeout(function() {
			...
			var html = '';
			html += '<div className="d-flex align-items-center">';
			html += '	<div className="symbol symbol-circle symbol-40 mr-3">';
			html += '		<img alt="Pic" src="assets/media/users/300_12.jpg"/>';
			html += '	</div>';
			html += '	<div>';
			html += '		<a href="#" className="text-dark-75 text-hover-primary font-weight-bold font-size-h6">Matt Pears</a>';
			html += '		<span className="text-muted font-size-sm">Just now</span>';
			html += '	</div>';
			html += '</div>';
			html += '<div className="mt-2 rounded p-5 bg-light-success text-dark-50 font-weight-bold font-size-lg text-left max-w-400px">';
        ...
        setTimeout(function() {
            $.ajax({
                ...
                error: function(res) {
                    ...
                    _hideProgress();
                    KTUtil.addClass(_target, _resultClass);
                    KTUtil.setHTML(_resultWrapper, '<span className="font-weight-bold text-muted">Connection error. Please try again later..</div>');
                    _showDropdown();
                    KTUtil.scrollUpdate(_resultWrapper);
                }
            });
